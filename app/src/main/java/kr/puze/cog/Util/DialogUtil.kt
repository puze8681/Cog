...
class DialogUtil(context: Context) {
    ...
    fun dialogPay(myPhone: String, cog: CogData) {
        ...
        dialog.button_dialog_pay.setOnClickListener { addPay(myPhone, cog, dialog.edit_dialog_pay.text.toString().toInt())}
    ...
    fun dialogEdit(myPhone: String, cog: CogData) {
        ...
        dialog.button_dialog_edit.setOnClickListener { editPay(myPhone, cog, dialog.edit_money_dialog_edit.text.toString().toInt(), dialog.edit_pay_dialog_edit.text.toString().toInt(), dialog.edit_count_dialog_edit.text.toString().toInt())}
        dialog.button_dialog_edit_cancel.setOnClickListener { dialog.dismiss() }
        dialog.show()
    }

    private fun editPay(myPhone: String, cog: CogData, money: Int, pay: Int, count: Int){
        val database: FirebaseDatabase = FirebaseDatabase.getInstance()
        val reference: DatabaseReference = database.getReference("Cogs")
        cog.money = money
        cog.pay = pay
